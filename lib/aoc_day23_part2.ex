defmodule AoC.Day23Part2 do
  ...
  def amphipod_done?(amphipods, amphipod, _origpos) do
    {amp, pos} = amphipod
    [first_pos, second_pos, third_pos, fourth_pos] = room(amp)

    first_pos == pos or
      ({amp, first_pos} in amphipods and second_pos == pos) or
      ({amp, first_pos} in amphipods and {amp, second_pos} in amphipods and third_pos == pos) or
      ({amp, first_pos} in amphipods and {amp, second_pos} in amphipods and {amp, third_pos} in amphipods and fourth_pos == pos)
  end
  ...
  def remove_rooms_if_in_same_room(positions, {_amp, origpos}) do
    Enum.reject(positions, fn {pos, _score} ->
      {orig_x_pos, _orig_y_pos} = origpos
      {x_pos, _y_pos} = pos
      orig_x_pos == x_pos and in_room?(origpos) and in_room?(pos)
    end)
  end
  ...
  def room("A"), do: [{3, 5}, {3, 4}, {3, 3}, {3, 2}]
  def room("B"), do: [{5, 5}, {5, 4}, {5, 3}, {5, 2}]
  def room("C"), do: [{7, 5}, {7, 4}, {7, 3}, {7, 2}]
  def room("D"), do: [{9, 5}, {9, 4}, {9, 3}, {9, 2}]
